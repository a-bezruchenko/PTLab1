...
class GetLastRatingQuartile():
    def calc(self, students_rating: RatingType) -> float:
        rating_list: List[float] = [students_rating[x]
        ...
        if length % 4 == 0:
            ...
            for i in range(length):
                if f_value_list[i] > 0.75:
                    ...
                    last_lower = i
            diff_pre_after: float = f_value_list[first_upper] -
                f_value_list[last_lower]
            diff_75: float = 0.75 - f_value_list[last_lower]
            ratio: float = diff_75 / diff_pre_after
            diff_pre_after_values: float = rating_list[first_upper] -
                rating_list[last_lower]
            quartile: float = rating_list[first_upper] +
                diff_pre_after_values * ratio
            return quartile
